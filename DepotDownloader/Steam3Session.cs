    ...
    class Steam3Session
    {
        ...
        private void DisconnectedCallback(SteamClient.DisconnectedCallback disconnected)
        {
            ...
            else if (!bAborted)
            {
                ...
                if (bConnecting)
                {
                    Console.WriteLine($"[Warning]|[ConnectionError]|Connection to Steam failed. Trying again (#{connectionBackoff})...");
                }
                else
                {
        ...
        private void LogOnCallback(SteamUser.LoggedOnCallback loggedOn)
        {
            ...
            if (isSteamGuard || is2FA || isAccessToken)
            {
                ...
                else if (isAccessToken)
                {
                    ...
                    Console.WriteLine($"[Error]|[TokenRejected]|Access token was rejected ({loggedOn.Result}).");
                    Abort(false);
                    return;
                }
                else
