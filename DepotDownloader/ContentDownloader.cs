...
namespace DepotDownloader
{
    ...
    static class ContentDownloader
    {
                    ...
                    do
                    {
                        ...
                        try
                        {
                            ...
                            if (manifestRequestCode == 0 || now >= manifestRequestCodeExpiration)
                            {
                                ...
                                {
                                    Console.WriteLine("[Error]|[NoManifestCode]|No manifest request code was returned for {0} {1}", depot.DepotId, depot.ManifestId);
                                    cts.Cancel();
                                }
                            }
                        ...
                        {
                            Console.WriteLine("[Warning]|[ConnectionTimeout]|Connection timeout downloading depot manifest {0} {1}. Retrying.", depot.DepotId, depot.ManifestId);
                        }
                        catch (SteamKitWebRequestException e)
                        {
                            ...
                            {
                                Console.WriteLine("[Error]|[{2}]|Encountered {2} for depot manifest {0} {1}. Aborting.", depot.DepotId, depot.ManifestId, (int)e.StatusCode);
                                break;
                            }
                            ...
                            Console.WriteLine("[Warning]|[Unknown]|Encountered error downloading depot manifest {0} {1}: {2}", depot.DepotId, depot.ManifestId, e.StatusCode);
                        }
                        catch (OperationCanceledException)
                        {
                            break;
                        ...
                        {
                            cdnPool.ReturnBrokenConnection(connection);
                            Console.WriteLine("[Warning]|[Unknown]|Encountered error downloading manifest for depot {0} {1}: {2}", depot.DepotId, depot.ManifestId, e.Message);
                        }
                    } while (newManifest == null);

                    if (newManifest == null)
                    {
                        Console.WriteLine("[Error]|[Unknown]|Unable to download manifest {0} for depot {1}", depot.ManifestId, depot.DepotId);
                        cts.Cancel();
                    }
            ...
            DepotManifest.ChunkData chunk)
        {
            ...
            try
            {
                do
                {
                    ...
                    catch (SteamKitWebRequestException e)
                    {
                        ...
                        if (e.StatusCode == HttpStatusCode.Unauthorized || e.StatusCode == HttpStatusCode.Forbidden)
                        {
                            Console.WriteLine("[Error]|[{1}]|Encountered {1} for chunk {0}. Aborting.", chunkID, (int)e.StatusCode);
                            break;
                        }

                        Console.WriteLine("[Warning]|[Unknown]|Encountered unexpected error downloading chunk {0}: {1}", chunkID, e.StatusCode);
                    }
                    catch (OperationCanceledException)
                    {
                        break;
                ...
                if (written == 0)
                {
                    Console.WriteLine("[Error]|[NoServer]|Failed to find any server with chunk {0} for depot {1}. Aborting.", chunkID, depot.DepotId);
                    cts.Cancel();
                }
            ...
            {
                var fileFinalPath = Path.Combine(depot.InstallDir, file.FileName);
                Console.WriteLine("[Info]|[Progress]|{0:#00.00}%|{1}|{2}", (sizeDownloaded / (float)depotDownloadCounter.completeDownloadSize) * 100.0f, SizeSuffix(sizeDownloaded), SizeSuffix(depotDownloadCounter.completeDownloadSize));
            }
